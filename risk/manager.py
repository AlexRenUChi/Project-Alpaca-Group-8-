...
class RiskManager:
    ...
    def check_order(self, intent: OrderIntent,
        ...
        if held_value + intent.notional > limits.max_position_notional:
            return False, (f"position would reach ${held_value + intent.notional:,.0f}, "
                           f"above per-asset cap ${limits.max_position_notional:,.0f}")

        gross = sum(abs(p.market_value) for p in positions.values())
        if gross + intent.notional > limits.max_gross_notional:
            return False, (f"gross exposure would reach ${gross + intent.notional:,.0f}, "
                           f"above cap ${limits.max_gross_notional:,.0f}")

        if held is None and len(positions) >= limits.max_positions:
            return False, f"already at max positions ({limits.max_positions})"

        if intent.notional > equity:
            return False, "order larger than account equity (no leverage allowed)"
